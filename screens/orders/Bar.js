...
import { FlatList, Image, Pressable, ScrollView, Text, TouchableOpacity, useWindowDimensions, View } from 'react-native';
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view';
import Modal from "react-native-modal";
import { Appbar, Searchbar } from 'react-native-paper';
import Toast from 'react-native-root-toast';
import styles, { default as styless } from '../../assets/css/style';
import BarIcon from '../../assets/images/bar_active.js';
...
export default function Bar({ navigation, route }: { navigation: any, route: any }) {
    ...
    if (JSON.stringify(result.data) === null || JSON.stringify(result.data) === "null" || result.data === "") {
      ...
      const filterData1 = result.data.map((category) => {
        const filteredItems = category.items.filter(
          (item) => item.itemStatus === "Order" && item.itemLocation === "Warehouse"
        );
      ...
      // Prepating Data starts from here
      // const filterPrepareData = result.data.filter((category) =>
      //   category.items.find((g) => g.itemStatus === "Prepared" && g.itemLocation === "WareHouse")
      // );
      const filterData2 = result.data.map((category) => {
        const filteredItems = category.items.filter(
          (item) => item.itemStatus === "Prepared" && item.itemLocation === "Warehouse"
        );
      ...
      //   category.items.find((g) => g.itemStatus === "Ready" && g.itemLocation === "WareHouse")
      // );
      const filterData3 = result.data.map((category) => {
        const filteredItems = category.items.filter(
          (item) => item.itemStatus === "Ready" && item.itemLocation === "Warehouse"
        );
      ...
      const filterDinerinorderData = result.data.filter((element) =>
        element.items.find((g) => element.orderType === "Dine-in" && g.itemLocation == "Warehouse")
      );
      //filter data by takeaway
      const filterWalkinorderData = result.data.filter((element) =>
        element.items.find((g) => element.orderType === "Walk-in" && g.itemLocation == "Warehouse")
      );
                  ...
                  <View>
                    <TouchableOpacity onPress={() => navigation.navigate('Bar')}>
                      <View style={[styles.dashboardHeader, styles.headerRightBorder, styles.dashMenuActive]}>
                        <BarIcon />
                        <Text style={[styles.font13, styles.priText, styles.fontBold, styles.padtop5]}>Bar</Text>
                      </View>
                    </TouchableOpacity>
