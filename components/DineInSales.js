...
export default function DineInSales({ data, updateDelete, sendEditData, ...props }) {
  ...
  const to = Math.min((page + 1) * numberOfItemsPerPage, data?.length || 0);
      ...
      {!Array.isArray(data) || data.length === 0 ? 
        ...
        <DataTable {...props}>
          <DataTable.Header style={[styles.headerStyle]}>
            ...
            <DataTable.Title><Text style={styles.tableHeader}>Ordered Date</Text></DataTable.Title>
          </DataTable.Header>
          <ScrollView>
            {data?.slice(
          ...
          <DataTable.Pagination
            page={page}
            numberOfPages={Math.ceil(data?.length / numberOfItemsPerPage) || 1}
            onPageChange={(page) => setPage(page)}
            label={`${from + 1}-${to} of ${data?.length || 0}`}
