...
export default function TableOutletView({ data, updateDelete, sendEditData, ...props }) {
    ...
    const callDeleteApi = async (outletId) => {
        const jsonValue = await AsyncStorage.getItem('userInfo')
        let loginData = JSON.parse(jsonValue);
        let token = loginData.token;
        let restaurantId = await AsyncStorage.getItem('restaurantId')

        const result = await api.DeleteMasterData(endPoint.DELETE_OUTLET + restaurantId + '/' + outletId, token);
    ...
    const callOutletEdit = async (account) => {
        sendEditData(account)
    }


    const tableRow = (account) => (
        <View style={[styles.tableRow, { borderColor: showView && showViewId == account.outletId ? '#407C6A' : '#F5F3F6' }]} >
            <DataTable.Row style={styles.datatableextraline} key={account.outletId}>
                <DataTable.Cell style={styles.userssl}><Text style={styles.tableCell}>{count++}</Text></DataTable.Cell>
                <DataTable.Cell><Text style={styles.tableCell}>{account.outletName}</Text></DataTable.Cell>
                <View style={styles.longDataTable}><Text style={styles.tableCell}>{account.outletAddress}</Text></View>
                <DataTable.Cell style={[styles.unitflex, styles.justifyCenter]}>
                    <View style={styles.tableButton}>
                        <View>
                            {
                                showViewId != account.outletId ?
                                    <TouchableOpacity style={styles.viewBtn} onPress={() => {
                                        setShowView(true)
                                        setShowViewId(account.outletId)
                                    }
                                    }>
                                        <EyeIcon />
                                    </TouchableOpacity>
                        ...
                        <TouchableOpacity onPress={() => callOutletEdit(account)} style={[styles.viewBtn, styles.editBtn]}>
                            <EditIcon />
                        </TouchableOpacity>
                        {/* <Pressable onPress={() => successOpenDelete(account.outletId)} style={styles.editBtn}>
                            <TableDelete />
                        </Pressable> */}
                        {data?.length > 1 ? (
                            <Pressable
                                onPress={() => successOpenDelete(account.outletId)}
            ...
            {
                showView && showViewId == account.outletId &&
                    ...
                    <View style={styles.tableViewBorder}>
                        <View style={styles.ViewConBlk}>
                            <View style={styles.viewCatView}>
                                ...
                                <Text style={styles.catValue}>{account.outletName}</Text>
                            </View>
                            <View style={styles.viewCatView}>
                                <Text style={styles.catName}>Outlet Location : </Text>
                                <Text style={[styles.catValue, styles.width70]}>{account.outletAddress}</Text>
                            </View>
            ...
            <View>
                <DataTable {...props}>
                    <DataTable.Header style={[styles.headerStyle]}>
                        ...
                        <DataTable.Title style={styles.flexOne}><Text style={styles.tableHeader}>Outlet Address</Text></DataTable.Title>
                        <DataTable.Title style={[styles.unitflex, styles.justifyCenter]}><Text style={styles.tableHeader}>Action</Text>
                        </DataTable.Title>
                    </DataTable.Header>
                    <ScrollView >
